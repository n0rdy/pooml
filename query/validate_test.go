...
func TestValidateRejects(t *testing.T) {
	tests := []struct {
		...
		{name: "table-valued function pragma", q: "SELECT * FROM pragma_table_info('logs')", wantMsg: "table-valued function"},
		{name: "table-valued function json_each", q: "SELECT * FROM json_each('[1,2,3]')", wantMsg: "table-valued function"},
		{name: "TVF joined to allowed table", q: "SELECT * FROM logs, pragma_database_list", wantMsg: "not allowed"},
		{name: "TVF inside subquery", q: "SELECT * FROM logs WHERE id IN (SELECT cid FROM pragma_table_info('logs'))", wantMsg: "table-valued function"},
